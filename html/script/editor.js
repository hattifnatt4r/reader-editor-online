...
var n_butset=5; var n_but=8; var n_x=4; var n_y=2; 
var l_x=100; var l_y=70; var o_x=0; var o_y=25; 
var d_x=0.6; var d_y=0.6;
...
function editor_show_start(){
	...
	inner_e1+= '<input id="editor_menu" type="button" class="buttons" value="menu" onclick="show_reader_menu();"  style="'+button_size_pos(0)[0]+'">';
...
function editor_scroll(order){
	...
	if (type==0) { symbol_arr=letters_arr; }  else if (type==1) { symbol_arr=letters_arr; }
	iter = parseInt(localStorage.getItem('editor_iter'));
	text = localStorage.getItem('text_edit');
	text_c = text.substr(0, iter)+symbol_arr[n]+text.substr(iter);
	localStorage.setItem('text_edit', text_c);
	iter_new = iter+1;
...
function editor_show_letters(type){
	...
	for (i=0; i<n_but; i++){
		...
		if (i+n*8 == n_butset*n_but-1){
			...
			inner_e += '<div id="editor_letter_'+i_name+'" class="buttons" onclick="editor_set_letter('+(i+n*n_but).toString()+','+type.toString()+');"  style="'+style+'">'+i_name+'</div>';
		}
	}
	elem.innerHTML = inner_e;
}
