...
echo 'LETTERS';
$arr_letters=array('a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z','-',',','space');
...
for ($i=0; $i<count($arr); $i++){
	if (isset($_POST[$arr[$i]])) {
		$word = $_SESSION["word_i"]; $c = $_SESSION["letter_counter"];
		if ($arr[$i]=='space'){ $new_word = substr($word, 0, $c)." ".substr($word,$c);}
		else{$new_word = substr($word, 0, $c).$arr[$i].substr($word,$c);}
